...
public:
    ...
    void dijkstras()
    {
        ...
        for (int count = 1; count < n; count++)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                if (!visited[i] && dist[i] > dist[v] + cost[v][i]) //change aahe
                {
...
//Cayley's Formula: It states that the number of spanning trees in a complete graph with N vertices is n raise to (n-2)